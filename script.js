// ========================================================
//  ГЛОБАЛЬНЫЙ ДВИЖОК ПЛАТФОРМЫ АКАДЕМИЯ ИСТОРИИ (V11.5)
// ========================================================

window.examVariantsDatabase = {
    "easy": {
        1: {
            title: "Вариант №1 (ЕГЭ База — Настоящая структура КИМ ФИПИ)",
            part1: [
                { 
                    id: 1, 
                    type: "matching",
                ...
                { 
                    ...
                    type: "matching",
                    num: "Задание 3", 
                    text: "Установите соответствие между процессами (явлениями, событиями) и фактами, относящимися к этим процессам:",
                    col1_title: "ПРОЦЕССЫ",
                    col2_title: "ФАКТЫ",
                ...
                { 
                    ...
                    type: "table",
                    num: "Задание 4", 
                    text: "Заполните пустые ячейки таблицы, используя приведённый ниже список пропущенных элементов:",
                ...
                { 
                    ...
                    type: "matching",
                    num: "Задание 5", 
                    text: "Установите соответствие между событиями (явлениями, процессами) и участниками этих событий:",
                    col1_title: "СОБЫТИЯ",
                ...
                { 
                    ...
                    type: "standard",
                    num: "Задание 6", 
                    text: "Прочтите отрывок из записок исторического деятеля:\n«Верховный Совет торжествовал победу. Власть перемещалась из рук государя в руки 12 неограниченных правителей... Обернувшись к Долгорукову, государыня сказала ему: \"Василий Лукич, ты меня, стало быть, обманул?\" и разорвала бумагу...».\n\nИспользуя отрывок, выберите верные суждения:\n1) Упоминаемая петиция содержала требование роспуска Сената.\n2) По свидетельству автора, победа верховников означала установление олигархического правления.\n3) Орган в первом абзаце создан в годы Петра I.\n4) Императрица, имя которой пропущено, была дочерью брата первого императора.\n5) Современниками событий были Бирон и Остерман.\n6) Сторонники самодержавия соединились с конституционалистами.", 
                    a: "2456" 
                },
                { 
                    id: 7, 
                    type: "matching",
                    num: "Задание 7", 
                    text: "Установите соответствие между памятниками культуры и их краткими характеристиками:",
                ...
                { 
                    ...
                    type: "standard",
                    num: "Задание 9", 
                    text: "<div class='textbook-map-wrapper'><img class='textbook-map-img' src='images/razin.jpg'></div>Рассмотрите схему. Указать правителя России в период, когда произошли события, обозначенные на схеме стрелками:", 
                    a: "алексеймихайлович" 
                },
                { id: 10, num: "Задание 10", text: "Укажите название города, обозначенного на схеме цифрой «2», в период, когда произошли отраженные события:", a: "симбирск" },
                { id: 11, num: "Задание 11", text: "Прочтите текст и укажите название города: «...решили идти на юг, чтобы захватить стоящий в дельте Волги торговый город — ___________».", a: "астрахань" },
                { id: 12, num: "Задание 12", text: "Какие суждения верны?\n5) Саратов находился на территории, охваченной восстанием.\n6) В городе под цифрой «1» было сформировано Второе народное ополчение.", a: "56" }
            ],
            part2: [
                { id: 13, num: "Задание 13", text: "Из доклада к Пленуму ЦК КПСС (1964 г.). Укажите год Пленума, пропущенную фамилию лидера (Хрущёв) и Председателя Совета Министров СССР в этом десятилетии.", c: "🔑 <b>Критерии ФИПИ:</b> 1) 1964 г. 2) Хрущёв. 3) А.Н. Косыгин." }
            ]
        }
    }
};

// Глубокое копирование вариантов для защиты от падения рендеринга
...
for (let v = 7; v <= 10; v++) { window.examVariantsDatabase.medium[v] = JSON.parse(JSON.stringify(window.examVariantsDatabase.medium[6])); }
for (let v = 12; v <= 15; v++) { window.examVariantsDatabase.hard[v] = JSON.parse(JSON.stringify(window.examVariantsDatabase.hard[11])); }
...
let currentTrainerSection = "russia"; let currentTrainerPeriod = "rus_ancient"; let currentTrainerDiff = "easy"; let currentTrainerQuestion = null; let globalLoadedQuestions = null;
...
// УМНЫЙ ДВИЖОК РЕНДЕРИНГА ТАБЛИЦ И СЕТОК ФИПИ ИЗ СКРИНШОТОВ
function startExamKIMSimulation(tier, variantNum) {
    ...
    document.getElementById('kim-part1-container').innerHTML = dataBlock.part1.map(q => {
        ...
        // Сборка двухколоночной таблицы сопоставления (Задания 1, 3, 5, 7)
        if (q.type === "matching") {
            questionContentHTML += `
            ...
            `;
            answerGridHTML = `
                <div class="fipi-letters-row">
                    ${q.col1.map(item => `<div class="fipi-letter-box">${item[0]}</div>`).join('')}
                </div>
            `;
        }

        // Сборка трехколоночной таблицы данных (Задание 4)
        if (q.type === "table") {
            questionContentHTML += `
                ...
                <div class="task-text-fipi" style="background:#f8fafc; padding:15px; border-radius:6px; border:1px solid #e2e8f0; font-size:14px;">${q.list}</div>
            `;
            answerGridHTML = `
                <div class="fipi-letters-row">
    ...
    globalLoadedQuestions = {
        "rus_ancient": {
            ...
            "hard": [
                ...
                { "q": "Как называлась древнейшая рукописная книга, созданная на Руси в 1056-1057 гг. по заказу новгородского посадника Остромира?", "a": "остромировоевангелие" },
