// ========================================================
//  ГЛОБАЛЬНЫЙ МОДЕРНИЗИРОВАННЫЙ ДВИЖОК ПЛАТФОРМЫ (V7.0)
// ========================================================
...
let currentActiveExamType = "ege"; let currentActiveKIM = null; let currentTrainerSection = "russia"; let currentTrainerPeriod = "rus_ancient"; let currentTrainerDiff = "easy"; let currentTrainerQuestion = null; let globalLoadedQuestions = null; let globalLoadedVariants = null;
...
function startExamKIMSimulation(tier, variantNum) {
    ...
    if (!globalLoadedVariants) {
        fetch('variants.json')
            ...
            .catch(() => alert('Файл КИМ variants.json отсутствует или поврежден!'));
    } else {
...
function initTrainerQuiz() {
    if (!globalLoadedQuestions) {
        fetch('questions.json')
            ...
            .catch(() => { alert('Файл тренажёра questions.json не найден!'); });
    } else {
...
function renderSingleQuestion() {
    ...
    if (!pool || pool.length === 0) {
        document.getElementById('quiz-question-text').innerText = "Для этого подраздела пул вопросов пуст."; return;
    }
    if (quizBlock) quizBlock.style.display = "block";
    currentTrainerQuestion = pool[Math.floor(Math.random() * pool.length)];
    document.getElementById('quiz-meta-info').innerText = `Период: ${currentTrainerPeriod.toUpperCase()} • Сложность: ${currentTrainerDiff.toUpperCase()}`;
